...
export default async function DashboardPage() {
  ...
  // 그룹 멤버들의 디바이스 조회 (main_viewer)
  let activeDeviceCount = 0;
  let lastUpdated: string | null = null;

  if (member.member_type === "main_viewer" && (memberCount ?? 0) > 0) {
    ...
    if (groupMembers && groupMembers.length > 0) {
      const memberIds = groupMembers.map((g) => g.member_id);
      ...
      if (devices && devices.length > 0) {
        activeDeviceCount = devices.filter((d) => d.is_on).length;
        ...
        if (latestLog?.logged_at) {
          const diff = Math.floor(
            (Date.now() - new Date(latestLog.logged_at).getTime()) / 60000
          );
          lastUpdated =
            diff < 1 ? "방금 전" : diff < 60 ? `${diff}분 전` : `${Math.floor(diff / 60)}시간 전`;
        }
      }
    }
  }
  ...
  const hasGroup =
    member.member_type === "main_viewer" ? (memberCount ?? 0) > 0 : !!myGroup;
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-2xl p-5 flex items-start gap-4">
          ...
          <div className="flex-1">
            ...
            <p className="text-sm text-blue-700 mt-1">
              초대 코드를 생성해 가족을 그룹에 추가하세요.
            </p>
          </div>
          <Link
            href="/mypage/group"
            className="flex items-center gap-1 text-sm font-semibold text-blue-600 hover:underline whitespace-nowrap mt-0.5"
          >
